...
import { TypeTd, TypeTdStat, TypeTh, } from "./";

// Pass stats reference to TypeTh and TypeTdStat it creates
// Its children components do not write data
const TypeStatsTable = React.memo(({ stats, typeDamage }) => (
  <table id="stats-table">
    <thead>
      <tr id="stats-head">
        <TypeTh variant="key-stats" />
        {typeDamage.map((row, index) => (
          <TypeTh key={index} variant="icon-fu" index={index} type={row.type} />
        ))}
        <TypeTh key="std-dev" variant="icon-fu" type="std-dev" title="Lower is better" />     
      </tr>
    </thead>
    <tbody>
